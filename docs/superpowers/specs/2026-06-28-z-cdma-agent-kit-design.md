...
将 z-cdma 的记忆框架升级为可被本地 Agent 日常直接使用的能力包。公共仓库发布的是接入、路由、沉淀和管理能力；用户的个人记忆、业务项目和私有 Skill 保留在本地 `storage/`。
...
- 不让公共框架直接决定业务内容产出。
- 不长期维护旧的本地目录结构作为推荐形态。
- 不在日常入口 Skill 中塞入备份、迁移、恢复等低频管理职责。
...
`storage/` 是唯一推荐的个人数据资产根目录。旧的 `domains/`、`skills_local/`、`knowledge/`、`content_projects/`、`assets_index/`、`publishing_local/`、`outputs/`、`renders/` 和 `tmp/` 只作为兼容迁移来源继续保持私有。
...
日常唯一入口。用于任何需要本地上下文、记忆、项目连续性或本地 Skill 的任务。
...
1. 审查 `memory/log/`、`memory/candidates/`、`memory/notes/` 和 `skills/`。
2. 合并重复或冲突记忆。
