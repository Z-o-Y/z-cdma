...
- A storage root.
- A review scope such as project, date range, memory topic, candidate file, or Skill folder.
- Existing `storage/memory/index.md` and `storage/skills/index.md`.
- Relevant files from `memory/log/`, `memory/candidates/`, `memory/notes/`, `memory/archive/`, and `skills/`.

## Workflow

1. Confirm the curation scope before reading broad memory.
2. Read the current digest and indexes.
3. Review only the logs, candidates, notes, and Skills inside scope.
